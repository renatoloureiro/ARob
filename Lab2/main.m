        ...
        case 3 % altitude estimation
            ...
            for i=1:6000
                hpp(i)=sin(euler.theta(i)*pi/180)*phys_accs(i,1)*g/1000 - ...
                    ...
                    cos(euler.phi(i)*pi/180)*cos(euler.theta(i)*pi/180)*phys_accs(i,3)*g/1000 + g;
            end
            ...
            [K_est,L]=kalman(sys, Q,R);
            name='altitude_stuff';
            out=estimate_stuff(navdata.time, altitude_vision.*(10^42),...
                double(transpose(hpp)),L,K_est, name, altitude_real.est_z);
...
elseif option==3
    [altitude_real, raw, euler, phys_accs,phys_gyros,altitude_vision,...
                altitude_vz,navdata]=trata_dados('EXP_D.mat');
     Q_w=[7 0 0; 0 6 0; 0 0 5]*(pi/180)^2;
     Q_b=(1/8)*Q_w;
...
function out=estimate_stuff(time, y, u ,L, K_est, name, real)
    ...
    in = in.setVariable('A_kalman', K_est.A);
